...
const SearchCombobox: React.FC<SearchComboboxProps> = ({ options, value, onChange, placeholder, isDark }) => {
  const inputRef = useRef<HTMLInputElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [search, setSearch] = useState('');
  const [open, setOpen] = useState(false);

  // CRITICAL SYNC + LOCALIZATION: Keep the inner search text in lockstep with
  // the parent form state, displaying the localized label (nameSinhala /
  // categorySi / sales type Sinhala) while storing the clean English value.
  useEffect(() => {
...
// ── Smart number helper: strips leading zeros, falls back to "0" on blur ──
function sanitizeNumericInput(raw: string): string {
  if (raw === '' || raw === '.') return raw;
  return raw.replace(/^0+(?=\d)/, '');
}

// ── Clearable input with leading-zero stripping ──
...
export const EditProductModal: React.FC<EditProductModalProps> = ({ item, onSave, onClose }) => {
  ...
  const [form, setForm] = useState<InventoryProduct>({ ...item });
  const [errors, setErrors] = useState<Record<string, string>>({});

  // 🚀 LOCALIZATION: Build category dropdown options as { value, label } pairs.
  // The raw English category name (c.name) is the stored value so the backend
  // payload stays unchanged. When Sinhala is active, the dropdown displays
  // nameSinhala / nameSi.
  ...
  // ── Derive status from storeQty whenever storeQty changes ──
  useEffect(() => {
    setForm((prev) => ({ ...prev, status: deriveStatus(prev.storeQty) }));
  }, [form.storeQty]);
  ...
  const handleSubmit = () => { if (validate()) onSave({ ...form, status: deriveStatus(form.storeQty) }); };

  const handleKeyDown = (e: React.KeyboardEvent) => { if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) handleSubmit(); };

  const handleStrUpdate = (key: string, val: string) => {
    // Allow empty string to pass through (for clear button)
    if (val === '') {
      updateField(key as any, '');
      return;
    }
    if (key === 'searchKey' || key === 'name' || key === 'productCategory') {
      updateField(key as any, val);
    } else {
      updateField(key as any, parseFloat(val) || 0);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div ref={modalRef} onKeyDown={handleKeyDown} className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl border shadow-2xl animate-slide-in ${isDark ? 'bg-slate-800 border-slate-700/50' : 'bg-white border-slate-200'}`}>
        {/* Header */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2">
              <FieldGroup label={t('addProductModal.category')} icon={<Layers className="w-3 h-3" />}>
                <SearchCombobox options={categoryOptions} value={form.productCategory} onChange={(v) => updateField('productCategory', v)} placeholder={t('addProductModal.categoryPlaceholder')} isDark={isDark} />
              </FieldGroup>
            </div>
            ...
            <div>
              <FieldGroup label={t('addProductModal.salesType')} icon={<ShoppingCart className="w-3 h-3" />}>
                <SearchCombobox options={salesTypeOptions} value={form.salesType} onChange={(v) => updateField('salesType', v as InventoryProduct['salesType'])} placeholder={t('addProductModal.salesTypePlaceholder')} isDark={isDark} />
              </FieldGroup>
            </div>
